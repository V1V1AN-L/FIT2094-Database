...
ALTER TABLE appointment ADD CONSTRAINT appt_pk PRIMARY KEY ( appt_no );

ALTER TABLE appointment
    ADD CONSTRAINT appt_uq UNIQUE ( appt_datetime,
                                    appt_roomno,
                                    patient_no,
                                    provider_code,
                                    appt_prior_apptno );
...
ALTER TABLE appointment
    ADD CONSTRAINT ck_appt_length CHECK ( appt_length IN ( 'S', 'T', 'L' ) );
...
ALTER TABLE emergency_contact ADD CONSTRAINT ec_uq UNIQUE ( ec_phone );
...
ALTER TABLE patient
    ADD CONSTRAINT ck_patient_state CHECK ( patient_state IN ( 'NT', 'QLD', 'NSW', 'ACT'
    , 'VIC',
                                                               'TAS', 'SA', 'WA' ) );
...
ALTER TABLE appointment
    ADD CONSTRAINT prior_appt_appt_fk FOREIGN KEY ( appt_prior_apptno )
